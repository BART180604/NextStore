...
export async function POST(req:NextRequest){
    ...
    try {
        ...
        return NextResponse.json({ error:`Webhokk Error: ${error}` }, {status:400});
    }
...
async function createOrderInSanity(
    ...
    const sanityProducts = lineItemsWithProduct.data.map((item) => {
        const stripeProductMetadata = (item.price?.product as Stripe.Product)?.metadata || {};
        
        return {
            _key: crypto.randomUUID(),
            product: {
                _type: 'reference',
                _ref: stripeProductMetadata.id,
            },
            quantity: item?.quantity || 0,
            color: stripeProductMetadata.color || "",
            size: stripeProductMetadata.size || "",
        };
    });
