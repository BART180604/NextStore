...
const CategoryProducts = ({categories,slug}:Props) => {
    ...
    const [currentSlug, setCurrentSlug] = useState(slug || categories?.[0]?.slug?.current || "")
        ...
        <div className=" flex flex-col md:min-w-40 border ">
            {categories?.map((item)=>(
                <Button 
                     ...
                     className={`bg-transparent border-0 rounded-none text-darkColor shadow-none hover:bg-darkColor/80 hover:text-white hoverEffect font-semibold border-b last:border-b-0 ${item?.slug?.current===currentSlug && "bg-darkColor text-white border-darkColor"} `}>
                        {item?.title}
                </Button>
            ))}
        </div>
